                ...
                <div className="mt-5 space-y-3">
                    {items.slice(0, 6).map((item, index) => (
                        <div
                            ...
                            <b>
                                {valueKeys[1] && item[valueKeys[1]] != null
                                    ? valueFormat(item[valueKeys[1]])
                                    : ""}
                            </b>
                        </div>
                    ))}
...
export default function AdminPage() {
    ...
    const saveProduct = async (event) => {
        ...
        const payload = {
            ...
            discountPrice:
                product.discountPrice === "" ? null : Number(product.discountPrice),
        };
        try {
    ...
    const metrics = [
        ["totalSales", "Total sales", (value) => formatPrice(value)],
        [
            "totalOrders",
            "Total orders",
    ...
    return (
        <section className="mx-auto max-w-7xl px-5 py-12 lg:px-12">
            ...
            <div className="mt-9 flex gap-6 overflow-auto border-b border-slate-200">
                ...
                <div className="mt-7 space-y-6">
                        ...
                        {metrics.map(([key, title, display]) => (
                            <article
                                ...
                                <p className="mt-4 font-serif text-4xl">
                                    {dashboard?.[key] == null ? "-" : display(dashboard[key])}
                                </p>
                            </article>
                        ))}
                    </div>
                    <div className="grid gap-6 lg:grid-cols-3">
                        <DashboardList
                            ...
                            items={dashboard?.salesOverview}
                            valueKeys={["label", "value"]}
                        />
                        <DashboardList
                            title="Orders overview"
                            items={dashboard?.ordersOverview}
                            valueKeys={["label", "value"]}
                        />
                        <DashboardList
                            title="Revenue by month"
                            items={dashboard?.revenueByMonth}
                            valueKeys={["month", "revenue"]}
            ...
            {tab === "products" && (
                <div className="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1fr)_380px]">
                                    ...
                                    <tr key={item.id} className="border-b last:border-0">
                                        <td className="p-4">
                                            ...
                                            <p className="text-xs text-slate-500">
                                                {item.brand || item.categoryName} · {item.sku}
                                            </p>
                                        </td>
                                        <td className="p-4">
                    ...
                    <form
                        ...
                        className="h-fit rounded-2xl bg-white p-6 ring-1 ring-slate-200">
                        ...
                        <div className="mt-5 grid gap-3">
                            {productFields.map(([key, label]) => (
                                <input
                                    key={key}
                                    required={!["discountPrice", "imageUrl"].includes(key)}
                                    type={
                                        ["price", "discountPrice", "stockQuantity"].includes(key)
                                            ? "number"
                                            : "text"
                                    }
                                    min={["price", "discountPrice"].includes(key) ? "0.01" : "0"}
                                    step={["price", "discountPrice"].includes(key) ? "0.01" : "1"}
                                    value={product[key]}
                                    onChange={(event) =>
                                        updateProductField(key, event.target.value)
                                    }
                                    placeholder={label}
                            ...
                            <textarea
                                ...
                                className="min-h-28 rounded-lg border px-3 py-2.5 text-sm"
                            />
                            <label className="flex items-center gap-2 text-sm">
                                <input
                                    ...
                                    onChange={(event) =>
                                        updateProductField("active", event.target.checked)
                                    }
                                />{" "}
                                Active product
                            </label>
                        ...
                        <button
                            disabled={saving}
                            className="mt-5 flex w-full items-center justify-center gap-2 rounded-lg bg-[#101726] px-6 py-3 text-sm font-bold text-white disabled:opacity-60">
